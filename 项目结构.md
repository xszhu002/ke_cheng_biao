...
这是一个基于Web的课程表管理系统，支持多教师课程表管理、拖拽换课、原始课程表保存与复位、特需托管等功能。系统采用前后端分离架构，使用原生技术栈实现，尽可能减少外部依赖。
...
├── server.js                   # 主服务器文件 (624行)
├── 设计方案.md                 # 原始设计文档
├── 项目结构.md                 # 本文档
├── CLAUDE.md                   # AI助手工作记录
│
├── database/                   # 数据库目录
│   └── schedule.db            # SQLite数据库文件 (40KB)
│
├── scripts/                   # 脚本目录
│   └── init-db.js            # 数据库初始化脚本 (160行)
│
├── public/                    # 前端静态文件
│   ├── index.html            # 主页面 (423行)
│   ├── favicon.ico           # 网站图标
...
│   │   ├── app.js           # 应用入口 (256行)
│   │   ├── api.js           # API客户端 (547行)
│   │   ├── schedule.js      # 课程表管理 (634行)
│   │   ├── dragdrop.js      # 拖拽功能 (512行)
│   │   ├── calendar.js      # 日历管理 (425行)
│   │   └── utils.js         # 工具函数 (575行)
│   │
│   └── assets/              # 静态资源目录 (空)
│
...
- **前端代码**：70% (HTML/CSS/JS)
- **后端代码**：25% (Node.js/Express)
- **数据库**：5% (SQLite/SQL)

这个项目展示了如何用最小的技术栈实现功能完整的Web应用，适合作为学习现代Web开发的实例，也适合实际的小规模课程表管理需求。
